...
def main():
    ...
    num_requested = 1000 # number of images to download
    ...
    for _ in xrange(number_of_scrolls):
	    ...
	    time.sleep(5)
            ...
            break
    time.sleep(5)
    #imges = driver.find_elements_by_tag_name('img')
    imges = driver.find_elements_by_xpath("//img[@class='rg_ic rg_i']")
    print "Total images:", len(imges), "\n"
    for img in imges:
        print(img)
        img_count += 1
	img_url = img.get_attribute('src')#json.loads(img.get_attribute('src'))
	img_type="jpg"
